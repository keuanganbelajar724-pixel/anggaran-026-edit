...
export function compactPengelolaanUPForFirestore(records: PengelolaanUPRecord[]): any[] {
  ...
  return records
    ...
    .map(r => {
      const isNihil = 
        r.kodeSatker === '693750' ||
        String(r.namaSatker || '').toUpperCase().includes('BINA MARGA') ||
        (String(r.keterangan || '').toUpperCase().includes('NIHIL') && !String(r.keterangan || '').toUpperCase().includes('BUKAN NIHIL')) ||
        (String(r.keteranganExcel || '').toUpperCase().includes('NIHIL') && !String(r.keteranganExcel || '').toUpperCase().includes('BUKAN NIHIL')) ||
        String(r.batasTeguran || '').toUpperCase().includes('NIHIL') ||
        String(r.statusRevolving || '').toUpperCase().includes('NIHIL') ||
        (Number(r.totalGUNihil) > 0 && Number(r.sisaUP) === 0);
      ...
      return {
        ...
        persentaseRevolving: Number(r.persentaseRevolving || r.persenRevolving) || 0,
        ...
        isJatuhTempo1Minggu: !isNihil && !!r.isJatuhTempo1Minggu,
        isOverdue: !isNihil && !!r.isOverdue,
        isHariLibur: !!r.isHariLibur,
        isNihil,
        keteranganExcel: String(r.keteranganExcel || ''),
        batasTeguran: String(r.batasTeguran || ''),
        totalGUNihil: Number(r.totalGUNihil) || 0,
        setoranUP: Number(r.setoranUP) || 0,
        saranTglPengajuan: String(r.saranTglPengajuan || ''),
        hariTanpaRevolving: Number(r.hariTanpaRevolving) || 0,
        peringatanKritis: !isNihil && !!r.peringatanKritis,
