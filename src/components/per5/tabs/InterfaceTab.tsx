...
export function getIndicatorDataStatus(key: string, project: SimulationProject): {
  ...
  switch (key) {
    case 'revisiDIPA': {
      ...
      if (rows.length === 0) {
        return {
          ...
          icon: 'empty',
          summary: 'Belum ada data revisi DIPA'
        };
      }
      const hasAnyData = rows.some(r => r.tanggalRevisi || (r.paguDipaMenjadi && r.paguDipaMenjadi > 0) || (r.paguMenjadi && r.paguMenjadi > 0) || r.jumlahDiperhitungkan > 0);
      if (!hasAnyData) {
        return {
          status: 'Belum diisi',
          badgeClass: 'bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700',
          icon: 'empty',
          summary: 'Formulir revisi kosong'
        };
      }
      return {
        ...
        icon: 'check',
        summary: `${rows.length} revisi tercatat`
      };
    }
    ...
    case 'deviasiHalIII': {
      ...
      const activeMonths = rows.filter(r => ((r.rencana51 || 0) + (r.rencana52 || 0) + (r.rencana53 || 0) + (r.penyerapan51 || 0) + (r.penyerapan52 || 0) + (r.penyerapan53 || 0)) > 0).length;
      if (activeMonths === 0) {
        return {
          status: 'Belum diisi',
          badgeClass: 'bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700',
          icon: 'empty',
          summary: 'RPD seluruh bulan 0'
        };
      }
      if (activeMonths < 12) {
        return {
    ...
    case 'pengelolaanUPTUP': {
      ...
      const hasTunai = tunaiRows.some(r => ((r.totalGUP || 0) + (r.totalOutstandingUP || 0) + (r.totalTUP || 0) + (r.totalSetoranTUP || 0)) > 0);
      const hasKKP = kkpRows.some(r => ((r.upKKPPerBulan || 0) + (r.penggunaanKKP || 0)) > 0);

      if (!hasTunai && !hasKKP && tunaiRows.length === 0 && kkpRows.length === 0) {
        return {
          status: 'Belum diisi',
          badgeClass: 'bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700',
          icon: 'empty',
          summary: 'Data UP/TUP & KKP kosong'
        };
      }
      return {
        status: 'Sudah dihitung',
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {onResetProjectToClean && (
              <button
                onClick={() => {
                  if (window.confirm('Kosongkan semua data dan mulai simulasi bersih dari 0?')) {
                    onResetProjectToClean();
                  }
                }}
                type="button"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/40 dark:hover:text-rose-400 hover:border-rose-200 transition-all cursor-pointer"
                title="Mulai Bersih dari 0"
              >
