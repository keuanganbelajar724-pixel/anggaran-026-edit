...
export const PenyerapanTab: React.FC<PenyerapanTabProps> = ({
  ...
  const rawInputs: PenyerapanInput[] = useMemo(() => {
    if (project.penyerapan && project.penyerapan.length >= 12) {
      return project.penyerapan;
    }
    // Jika kosong, inisialisasi dari DEFAULT_EXCEL_PENYERAPAN_PERIODS (12 periode)
    return DEFAULT_EXCEL_PENYERAPAN_PERIODS.slice(0, 12).map((r: any) => ({
      periode: r.periode,
      pagu51: r.pagu51 ?? 0,
      pagu52: r.pagu52 ?? 0,
      pagu53: r.pagu53 ?? 0,
  ...
  const result = calculationOutput.result;
  const warnings = calculationOutput.warnings;

  // Nilai ringkasan triwulan:
  // TW I = Q17 (periode '03', index 2)
  // TW II = Q35 (periode '06', index 5)
  // TW III = Q53 (periode '09', index 8)
  // TW IV = Q71 (periode '12', index 11)
  const scoreTW1 = periods[2]?.nilaiIndikator ?? 0;
  const scoreTW2 = periods[5]?.nilaiIndikator ?? 0;
  const scoreTW3 = periods[8]?.nilaiIndikator ?? 0;
  const scoreTW4 = periods[11]?.nilaiIndikator ?? 0;
  const finalScore = result.cappedValue; // Q71
  ...
  const handleResetToWorkbook = () => {
    if (window.confirm('Reset data Penyerapan Anggaran ke default workbook "Kalkulator Perhitungan IKPA 2026.xlsx"?')) {
    ...
    reader.onload = evt => {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length >= 12) {
          ...
          alert('Berhasil mengimpor 12 periode data Penyerapan Anggaran!');
        } else {
          alert('Format file JSON tidak valid. Memerlukan array minimal 12 periode.');
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="text-right px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700">
            ...
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
              Bobot 20%: {result.weightedValue.toFixed(2)}
            </span>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center rounded-xl bg-slate-100 dark:bg-slate-800 p-1 text-xs font-semibold">
          ...
          >
            ...
            Tampilan Excel (12 Periode Lengkap)
          </button>
          <button
      ...
      {/* 7. VIEW MODE 1: TAMPILAN EXCEL (12 PERIODE LENGKAP) */}
      {viewMode === 'excel' && (
        <div className={`rounded-2xl border overflow-hidden shadow-xs ${
          ...
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h4 className="font-bold text-sm text-slate-900 dark:text-slate-100">
                Struktur Buku Kerja Excel: Penyerapan Anggaran (12 Periode)
              </h4>
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      {viewMode === 'simple' && (
        <div className="space-y-5">
          ...
          {periods[selectedMonthIdx] && (() => {
            const curP = periods[selectedMonthIdx];
            const curIdx = selectedMonthIdx;
