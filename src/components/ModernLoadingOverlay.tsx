...
import { Loader2, Sparkles, Database, ShieldCheck, Cpu } from 'lucide-react';
...
export const ModernLoadingOverlay: React.FC<ModernLoadingOverlayProps> = ({
  ...
  const content = (
    <AnimatePresence>
      <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-2xl">
        <motion.div
          initial={{ opacity: 0, scale: 0.9, y: 15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: -10 }}
          transition={{ type: 'spring', stiffness: 350, damping: 25 }}
          className={`relative w-full max-w-md rounded-3xl p-8 shadow-2xl border text-center overflow-hidden ${
            isDark
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden p-0.5 border border-slate-200 dark:border-slate-700/80">
                <motion.div
                  ...
                  className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-cyan-400 rounded-full shadow-md"
                />
              </div>
            </div>
          )}
